...
1. 默认行为应该是“按当前软件取数”：
   ...
   - Kimi Code（Kimi CLI）下用 `--agent-tool kimi-code`，读 `~/.kimi/sessions/…/context.jsonl`（或 `KIMI_SHARE_DIR`）里 `_usage.token_count`，这是**上下文占用累计**，不是 API 分项；票面 `USD ESTIMATE` 会正直地显示为 `UNMAPPED`，分项 Input/Output 行会省略。
   - 不允许因为别的软件日志更新得更晚，就跨软件偷读。
2. 如果当前运行环境识别不出软件，而且本机同时存在多套日志，必须显式传 `--agent-tool codex`、`--agent-tool claude-code` 或 `--agent-tool kimi-code`；不要猜。
...
- `token_receipt/data.py`
  - 负责读取 Codex JSONL、Claude usage-data、Claude transcript model、Kimi Code context.jsonl、价格表和字段能力报告。
